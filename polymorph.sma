...
#define VOTE_START "sound/sourcevote/startyourvoting.mp3"
#define VOTE_END "sound/sourcevote/endofvote.mp3"
...
{
	precache_generic(VOTE_START);
	precache_generic(VOTE_END);
	precache_sound(SOUND_SELECT);
}

public plugin_init()
{
	register_plugin("Megamod", "0.0.1", "Tirant")
	register_cvar("Megamod", "Tirant", FCVAR_SERVER|FCVAR_SPONLY)
...
public fw_getGameDescription() {
	forward_return(FMV_STRING, "Minigame Party");
	return FMRES_SUPERCEDE;
}

native csdm_set_intromsg(on_or_off);
...
public sayNextmod()
{
	...
	client_print_color(0, DontChange, "^1Next Mod: ^4%s", g_szModNames[g_iNextMod])
...
public sayThismod()
{
	...
	client_print_color(0, DontChange, "^1This Mod: ^4%s", g_szModNames[g_iThisMod])
...
public startModVote()
{
	...
	client_print_color(0, DontChange, "^4It's time to choose the next mod...")
	// client_print(0, print_chat, "%L", LANG_SERVER, "TIME_CHOOSE_MOD") // ML
	//client_cmd(0, "spk Gman/Gman_Choose2")
	client_cmd(0, "mp3 play %s", VOTE_START);
...
public countModVotes(id, key)
{
	...
	if (get_pcvar_num(g_pVoteAnswers))
	{
		...
		if (key == SELECTMODS)
			client_print_color(0, DontChange, "^3%s ^1chose ^4mod extending", name)
			// client_print(0, print_chat, "%L", LANG_PLAYER, "CHOSE_EXT_MOD", name) // ML
		else if (key < SELECTMODS)
			client_print_color(0, DontChange, "^3%s ^1chose ^4%s", name, g_szModNames[g_nextModId[key]])
	}
	++g_voteModCount[key]
...
public checkModVotes()
{
	...
	if (g_voteModCount[SELECTMODS] > g_voteModCount[b] )
	{
		...
		client_print_color(0, DontChange, "^4%s ^1has been extended for ^4one ^1map", g_szModNames[g_iNextMod])
	...
	else
	{
		...
		client_print_color(0, DontChange, "^3Choosing finished. The nextmod will be ^4%s", g_szModNames[g_iNextMod])
	...
	client_cmd(0, "mp3 play %s", VOTE_END);
...
public startMapVote()
{
	...
	new pos = format(menu, 511, g_coloredMenus ? "\yChoose the nextmap:\w^n^n" : "Choose the nextmap:^n^n")
	...
	client_print_color(0, DontChange, "^4It's time to choose the nextmap...")
	//client_cmd(0, "spk Gman/Gman_Choose2")
	client_cmd(0, "mp3 play %s", VOTE_START);
...
public countMapVotes(id, key)
{
	...
	if (get_pcvar_num(g_pVoteAnswers))
	{
		...
		if (key == SELECTMAPS)
			...
			client_print_color(0, DontChange, "^3%s ^1chose ^4map extending", name)
		else if (key < SELECTMAPS)
		{
			...
			client_print_color(0, DontChange, "^3%s ^1chose ^4%s", name, map)
		}
	}
	++g_voteMapCount[key]
...
public checkMapVotes()
{
	    ...
	    && g_voteMapCount[SELECTMAPS] > g_voteMapCount[SELECTMAPS+1])
	{
		...
		client_print_color(0, DontChange, "^3Choosing finished. Current map will be extended to next ^4%.0f ^3minutes", steptime)
	...
	//client_print(0, print_chat, "^4%L", LANG_PLAYER, "CHO_FIN_NEXT", smap)
	client_print_color(0, DontChange, "^3Choosing finished. The nextmap will be ^4%s", smap)
